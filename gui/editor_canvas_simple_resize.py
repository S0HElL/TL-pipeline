"""
Manga Translator GUI - Simple Resizable Editor Canvas
Interactive image display and text editing canvas with simple, working resizeable bounding boxes.
"""
...
class SimpleBoundingBoxItem(QGraphicsRectItem):
    """
    Simple resizable bounding box for text regions.
...
class SimpleEditorCanvas(QGraphicsView):
    """
    Simple main canvas for image display and text editing.
    
    Features:
    - Image loading and display with zoom/pan
    - Simple resizable bounding box editing
    ...
    def create_bounding_boxes(self):
        ...
        for i, data in enumerate(self.translated_data):
            ...
            box_item = SimpleBoundingBoxItem(rect, data)
    ...
    def mousePressEvent(self, event):
        ...
        if event.button() == Qt.LeftButton:
            ...
            clicked_box = None
            for item in items:
                if isinstance(item, SimpleBoundingBoxItem):
                    clicked_box = item
                    break
    ...
    def wheelEvent(self, event):
        ...
        if event.modifiers() == Qt.ControlModifier:
            ...
            self.verticalScrollBar(). self.verticalScrollsetValue(
               Bar().value() + int(delta.y())
            )
